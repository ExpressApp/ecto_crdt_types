defmodule EctoCrdtTypes.Types.State.AWSetTest do
  ...
  describe "#add_all/3" do
    test "adds values to crdt" do
      assert {:state_awset, {[{1, [a: 1]}, {2, [a: 2]}], {[a: 2], []}}} = AWSet.new() |> AWSet.add_all([1, 2], :a)
    end
  end

  describe "#rmv/3" do
    test "removes value from crdt" do
      assert {:state_awset, {[{2, [a: 2]}], {[a: 2], []}}} = AWSet.new([1, 2], :a) |> AWSet.rmv(1, :a)
    end
  end
  describe "#rmv_all/3" do
    test "removes value from crdt" do
      assert {:state_awset, {[], {[a: 2], []}}} = AWSet.new([1, 2], :a) |> AWSet.rmv_all([1, 2], :a)
    end
  end
end
